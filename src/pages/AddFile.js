...
const AddFile = () => {
  ...
  return (
    <div className="h-[90vh] py-4 flex flex-col items-center justify-center">
      ...
      >
        {isFile ? (
          fileData.map((file, idx) => (
            <div
              className="h-[150px] overflow-hidden w-[200px] p-2 flex justify-between flex-col items-start shadow-lg"
              key={idx}
            >
              <h3
          ...
          <h2 className="font-serif text-xl text-gray-700">
            {localStorage.getItem("language") === "arabic"
              ? "لتحميل ملف ، انقر أو اسحب في هذه المنطقة."
              : "To Upload file, Click or Drag in this area."}
          </h2>
        )}
        <input
          multiple
          ...
          ref={inputRef}
          onChange={(e) =>
            e.target.files.length > 0 && setData([...e.target.files])
          }
        />
      </div>
      <Button
      ...
      >
     {
            localStorage.getItem("language") === "arabic" ? "تحميل" : "Upload"
     }
      </Button>
    </div>
  );
